...
1. Run `V2.5 content cleanup` only if user testing still supports it: replace hardcoded Colin examples and revisit early mission estimate copy as a narrow content pass.
2. Keep date-keyed daily sessions, new mission types, richer SRS intervals, and broader content-model changes deferred unless a later slice explicitly accepts that larger model change.

...
1. Run `V2.4 early output and grammar pedagogy` from `Japanese_OS_feedback_pass_2_recording_3_plan.md`: audit early output scaffolding and Common Mistakes placement, then implement only the smallest high-confidence copy/support/layout improvement that fits current schemas.
2. Then run `V2.5 content cleanup` only if user testing still supports it: replace hardcoded Colin examples and revisit early mission estimate copy as a narrow content pass.
3. Keep date-keyed daily sessions, new mission types, richer SRS intervals, and broader content-model changes deferred unless a later slice explicitly accepts that larger model change.

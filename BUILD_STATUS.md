...
1. Audit early-listening prep pacing after user testing and decide whether any first-run listening mission still needs delayed placement.
2. Audit listening supported-exposure wording and weak-point impact after user testing.
3. Audit grammar-focus highlight coverage after more user testing and add explicit metadata only if derived highlights prove too noisy.
4. Begin the next narrow personalization slice only after the current mobile loop still feels clean under user testing.
...
- Listening flow follow-up: first-run listening now opens with a compact hear-the-pattern prep screen before checks, and the check screen still uses progressive hints; we should still evaluate deeper audio-first scoring, whether meaning hints should unlock only after an answer attempt, and how much post-check support should surface automatically.
...
- Grammar focus highlighting: completed in the Phase 4 grammar-focus cleanup by deriving subtle highlight terms from existing grammar lesson titles/ids and rendering them inline inside grammar examples without schema changes; the follow-up cleanup removed redundant focus chips.
...
- Listening hint/scoring semantics: completed in the Phase 4 listening semantics cleanup by treating answer reveals as supported exposure that preserves flow while keeping or adding review pressure instead of clearing mastery.
- Early listening prep flow: completed in the Phase 4 early-listening prep cleanup by keeping listening early in the path but adding a hear-the-pattern screen before first-run checks using existing support examples and audio.

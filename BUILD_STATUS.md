...
1. Add subtle grammar-focus highlighting inside grammar examples so learners can see the target pattern or particle quickly without turning examples into heavy annotations.
2. Design and then implement listening hint/scoring semantics so final answer reveals count as supported exposure rather than full mastery, while preserving local-first progress.
3. Revisit early listening placement and prep flow: decide whether listening should enter after one more exposure or include a short mandatory hear-the-pattern screen before checks.
4. Design the guided Today lesson-track presentation layer so completion recaps can intentionally route to the next visible Today action without adding backend scheduling.
...
- Guided Today lesson track: accepted as a product-direction candidate, but deferred until a narrow presentation-layer design exists for chaining visible Today actions without changing recommendation semantics.

...
1. Fix Today resume precedence and deduping so an active `Pick up where you stopped` mission appears above the daily plan and does not duplicate inside `Do this today`.
2. Add subtle grammar-focus highlighting inside grammar examples so learners can see the target pattern or particle quickly without turning examples into heavy annotations.
3. Design and then implement listening hint/scoring semantics so final answer reveals count as supported exposure rather than full mastery, while preserving local-first progress.
4. Revisit early listening placement and prep flow: decide whether listening should enter after one more exposure or include a short mandatory hear-the-pattern screen before checks.

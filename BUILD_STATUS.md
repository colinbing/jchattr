...
- User can complete 29 starter missions across 4 mission types:
  - 8 grammar
  - 8 listening
  - 8 output
  - 5 reading
...
- Pack 8 listening items are audio-ready in content, but their matching MP3 files are not generated yet
...
1. Generate MP3 files for the 5 new pack-8 listening items so Settings regains full audio coverage for current content.
...
- Listening items may include `audioRef`; all 39 current listening items point to static files under `public/audio/listening`
- Matching MP3 files currently exist for 34 listening items
- Before this pass, this file was stale: the repo already had full generated audio coverage through pack 7, but the status text still claimed pack-7 files were missing
- The 5 pack-8 listening items are audio-ready in content but their matching MP3 files are not yet present in `public/audio/listening`
- Settings now reports manifest-backed listening coverage from the current generated asset list, so pack-8 audio will appear as missing until files are generated and the manifest is synced
